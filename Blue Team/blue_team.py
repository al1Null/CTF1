...
from monitor import RobustMonitor as check_robustness
...
def log_traces(elapsed_time, traces, old_traces, water_volume_ml, red_ml, blue_ml):
    ...
    # Display formatting
    max_width = max(max(map(len, map(lambda x: "{:07.3f}".format(x), traces))),
                    max(map(len, map(lambda x: "{:07.3f}".format(x), diff))))
    ...
    values = "{:<13.2f} | {:<3.2f} | {:<3.2f} | {:<3.2f} | ".format(elapsed_time, water_volume_ml, red_ml,
                                                                    blue_ml) + " | ".join(
        [format_str.format("{:07.3f}".format(val)) for val in traces])
    rate_diff = "Rate Diff     |      |      |      | " + " | ".join(
        [format_str.format("{:07.3f}".format(val)) for val in diff])
